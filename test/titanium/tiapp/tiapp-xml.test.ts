...
describe('TiappXML', () => {
	...
	// 	const tiapp = new TiappXML();
	// 	await expect(tiapp.load(join(fixturesDir, 'does_not_exist'))).rejects.toThrow('File not found: does_not_exist');
	// });

	// it('should error selecting if no file loaded', async () => {
	// 	const tiapp = new TiappXML();
	// 	await expect(tiapp.select1()).rejects.toThrow('No tiapp.xml loaded');
	// });

	// it('should error loading if file is malformed', async () => {
	// 	const tiapp = new TiappXML();
	// 	await expect(tiapp.load(join(fixturesDir, 'bad.xml'))).rejects.toThrow('ParseError: unclosed xml tag(s): ti:app');
	// });
});
