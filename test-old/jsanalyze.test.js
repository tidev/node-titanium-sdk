...
describe('jsanalyze', () => {
	describe('#analyzeJs()', () => {
		...
		it('converts global "this" references into "global" references when transpiling', () => {
			const results = analyzeJs('this.myGlobalMethod = function() {};', {
				transpile: true,
			});
		...
		it('doesn\'t converts function-scoped "this" references into "global" references when transpiling', () => {
			...
			expect(results.contents).toEqual(
				'var myGlobalMethod = function myGlobalMethod() {return this;};'
			);
		});
		...
		it('generates source maps inline into generated js file', () => {
			...
			const expectedBase64Map = Buffer.from(JSON.stringify(expectedSourceMap)).toString(
				'base64'
			);
			expect(results.contents).toEqual(
		...
		it('handles input JS file with existing sourceMappingURL pointing to file', () => {
			...
			let expectedSourceMap = JSON.parse(
				fs.readFileSync(path.join(__dirname, 'resources/intermediate.js.map'), 'utf8')
			);
		...
		it('handles input JS file with existing sourceMappingURL pointing to non-existent file', () => {
			...
			const inputJSFile = path.join(
				__dirname,
				'resources/input.nonexistent.sourcemapfile.js'
			);
			const contents = fs.readFileSync(inputJSFile, 'utf-8');
			let expectedSourceMap = JSON.parse(
				fs.readFileSync(
					path.join(__dirname, 'resources/input.nonexistent.sourcemapfile.js.map'),
					'utf8'
				)
			);
			expectedSourceMap.sourceRoot = path.dirname(inputJSFile);
		...
		it('converts OS_IOS into boolean', () => {
			const results = analyzeJs('if (OS_IOS) {}', {
				transpile: true,
				transform: { platform: 'ios' },
			});
		...
		it('should handle errors', () => {
			expect(() => analyzeJs('return foo!;console.log("bar");')).toThrow(
				'Failed to parse undefined\nMissing semicolon. (1:10)'
			);
		});
	});
