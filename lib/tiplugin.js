 ...
 */
export async function scopedDetect(searchPaths, config, logger) {
 ...
 * @returns {void}
 */
export async function detect(projectDir, config, logger, bypassCache) {
	...
	pluginCache = Object.assign({}, ...results);
	return pluginCache;
}
 ...
 * @returns {Object}
 */
export async function find(plugins, searchPaths, config, logger) {
	...
	const installed = typeof searchPaths === 'string'
		// searchPaths is the project directory
		? await detect(searchPaths, config, logger)
		// searchPaths is an object of paths
		: await scopedDetect(searchPaths, config, logger);
	...
	for (const plugin of plugins) {
		...
		const scopes = [ 'project', 'config', 'user', 'global' ]; // the order here represents precendence ('user' is legacy, now we use 'config')
...
async function detectPlugins(searchPaths, config, logger) {
	...
	for (let pluginRoot of searchPaths) {
		pluginRoot = expand(pluginRoot);
		...
		for (const name of fs.readdirSync(pluginRoot)) {
			const pluginsPath = path.join(pluginRoot, name);
			if (!fs.existsSync(pluginsPath) || !fs.statSync(pluginsPath).isDirectory() || ignoreDirs.test(name)) {
				continue;
			}
			...
			const processDir = async (ver, versionPath, dest) => {
				...
				if (fs.existsSync(hooksDir) && fs.statSync(hooksDir).isDirectory()) {
					for (const filename of fs.readdirSync(hooksDir)) {
						...
						if (fs.statSync(file).isFile() && jsfile.test(filename) && !ignore.test(filename)) {
							...
							try {
								vm.runInThisContext(`(function (exports, require, module, __filename, __dirname) { ${fs.readFileSync(file, 'utf8')}\n});`, file, 0, false);
				...
				if (ver) {
					logger?.debug(`Detected plugin: ${name} ${ver} @ ${versionPath}`);
				} else {
					logger?.debug(`Detected plugin: ${name} @ ${versionPath}`);
				}
			};
