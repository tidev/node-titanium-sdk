...
export class EmulatorManager {
	...
	detect(opts, callback) {
		...
		Promise.all(files.map(async (filename) => {
			...
			android.detect(this.config, opts, (androidEnv) => {
				const ver2api = {};
				const emus = [];
	...
	checkedBooted(config, opts, emulator) {
		...
		emulator.on('booted', async () => {
			...
			adb.shell(deviceId, 'ls -l /sdcard', async (err, output) => {
				...
				opts.logger?.debug(`Checking mount points: ${mountPoints.join(', ').cyan}`);
	...
	start(id, opts, callback) {
		...
		opts.logger?.trace(`Checking if emulator ${id.cyan} is running...`);
