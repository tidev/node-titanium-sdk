...
EmulatorManager.prototype.isRunning = function isRunning(id, opts, callback) {
	...
	this.detect(opts, function (err, emus) {
		...
		if (!emu) {
			return callback(new Error(__('Invalid emulator "%s"', id)), null);
		}
				...
				adb.shell(deviceId, 'ls -l /sdcard', function (err, output) {
					...
					async.whilst(
						...
						function () {
							...
							adb.devices(function (err, devices) {
								emulib.isRunning(config, emu, devices.filter(d => d.id = emulator.id), function (err, running) {
									if (!err && running) {
										appc.util.mix(emulator, running);
									}
									emulator.emit('ready', emulator);
...
EmulatorManager.prototype.start = function start(id, opts, callback) {
	...
	this.isRunning(id, opts, function (err, running) {
		...
		this.detect(opts, function (err, emus) {
			...
			var emu = emus.filter(e => e && e.id == id).shift(); // eslint-disable-line eqeqeq
...
EmulatorManager.prototype.stop = function stop(id, opts, callback) {
	...
	this.isRunning(id, opts, function (err, running) {
		if (err) {
			...
			// already stopped
			callback(new Error(__('Emulator "%s" not running', id)));
		} else {
			require(path.join(__dirname, 'emulators', running.emulator.type + '.js')).stop(this.config, running.emulator.name, running, opts, callback);
		}
	}.bind(this));
};
