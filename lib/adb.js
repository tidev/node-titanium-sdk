...
export class Connection {
	...
	exec(cmd, callback, opts) {
				...
				switch (this.state) {
					case WAIT_FOR_COMMAND_RESULT:
						...
						if (!/^OKAY|FAIL$/.test(result)) {
							callback(new Error(`Unknown adb result "${result}"`));
							return;
						}
		...
		socket.on('error', (err) => {
			...
			this.adb.startServer((code) => {
				if (code) {
					callback(new Error(`Unable to start Android Debug Bridge server (exit code ${code})`));
				} else {
...
function androidDetect(config, callback) {
	import('./android.js').then(({ detect }) => {
		detect(config, null, (results) => {
...
export class ADB {
	...
	version(callback) {
		...
		conn.exec('host:version', (err, data) => {
			if (err) {
				return callback(err);
			}
	 ...
	 */
	parseDevices(adb, callback, err, data) {
		if (err) {
			callback(err);
			return;
		}
		...
		Promise.all(data.toString().split('\n').map(async line => {
			...
			return new Promise((resolve) => {
				adb.shell(info.id, 'getprop', (err, data) => {
					if (!err && data) {
						const re = /^\[([^\]]*)\]: \[(.*)\]\s*$/;
						data.toString().split('\n').forEach(line => {
	 ...
	 */
	devices(callback) {
		new Connection(this).exec('host:devices', (err, data) => {
	...
	startServer(callback) {
		androidDetect(this.config, function (err, results) {
			...
			child.on('close', (code) => {
				callback(code ? new Error(`Failed to start ADB (code ${code}): ${stderr}`) : null);
			});
		});
	}
	...
	installApp(deviceId, apkFile, opts, callback) {
		...
		this.devices((err, devices) => {
			...
			androidDetect(this.config, (err, results) => {
				...
				opts.logger?.trace(`Executing: ${[ results.sdk.executables.adb ].concat(args).join(' ').cyan}`);
				...
				child.on('close', (code) => {
					...
					if ((code && stderr.indexOf('No space left on device') !== -1) || (!code && m && m[1] === 'INSTALL_FAILED_INSUFFICIENT_STORAGE')) {
