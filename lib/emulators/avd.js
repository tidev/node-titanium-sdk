...
export function detect(config, opts, callback) {
	opts = opts || {};
	detectAndroid(config, opts, (results) => {
 ...
 * @returns {void}
 */
export function isRunning(_config, emu, devices, callback) {
	if (emu.type !== 'avd') {
...
export function start(config, emu, opts, callback) {
	...
	detectAndroid(config, { bypassCache: opts.bypassCache }, async (results) => {
		if (!results.sdk) {
		...
		opts.logger?.info(`Running: ${(`${results.sdk.executables.emulator} "${args.join('" "')}"`)}`);
