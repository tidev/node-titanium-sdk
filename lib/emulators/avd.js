		...
		async.whilst(
			...
			function (cb) {
				var socket = net.connect({ port: tryPort }, function () {
					...
					socket.end();
					tryPort++;
					cb(tryPort > 5584 ? new Error('Unable to find a free port between 5554 and 5584') : null);
				});
			...
			function (err) {
				...
				opts.logger && opts.logger.trace(`Emulator will listen on port ${String(port).cyan}`);
				...
				opts.logger && opts.logger.info(`Running: ${(results.sdk.executables.emulator + ' "' + args.join('" "') + '"').cyan}`);
