...
export class Builder {
	conf = {};
	buildDirFiles = {};
	titaniumSdkVersion = ti.manifest.version;
	...
	constructor(buildModule) {
		this.titaniumSdkName = this.locateSDKPath();
		...
		this.packageJson = JSON.parse(fs.readFileSync(path.join(this.platformPath, 'package.json'), 'utf8'));
	}
	...
	locatePlatformPath(buildModule) {
		let dir = path.dirname(buildModule.filename);
		const { root } = path.parse(dir);
		while (dir !== root) {
		...
		if (fs.existsSync(this.buildDir)) {
			...
			while (dirsToProcess.length > 0) {
				...
				for (const name of fs.readdirSync(currentDir)) {
					const file = path.join(currentDir, name).normalize();
					try {
	...
	*/
	 ...
	 */
	validateTiModules(platformName, deployType, callback) {
		...
		appc.timodule.find(this.cli.tiapp.modules, platformName, deployType, ti.manifest, moduleSearchPaths, this.logger, (modules) => {
	 ...
	 */
	generateAppIcons(icons, callback) {
		...
		this.logger.debug(`Found ${defaultIcon.cyan} (${size.width}x${size.height})`);
		if (icons.length) {
		...
		for (let i = 0; i < icons.length; i++) {
			...
			if (icon.required) {
				...
				this.logger.warn(`${defaultIcon} (${size.width}x${size.height}) is not large enough to generate missing icon "${path.basename(icon.file)}" (${icon.width}x${icon.height}), skipping`);
				icons.splice(i--, 1);
				continue;
			}
		...
		appc.image.resize(defaultIcon, icons, (error, _stdout, _stderr) => {
			if (error) {
