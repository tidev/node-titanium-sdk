...
export function scopedDetect(searchPaths, _config, logger, callback, bypassCache) {
 ...
 */
export function find(modulesOrParams, platforms, deployType, tiManifest, searchPaths, logger, callback, bypassCache) {
	const result = {
			found: [],
			missing: [],
			incompatible: [],
	...
	detect({
		...
		callback: function (installed) {
			params.modules && params.modules.forEach(function (module) {
				...
				params.logger?.debug(`Looking for Titanium module id=${cyan(module.id)} version=${cyan(originalVersion)} platform=${cyan(module.platform.join(','))} deploy-type=${cyan(module.deployType.join(','))}`);
				...
				for (let i = 0; i < scopes.length; i++) {
					...
					for (let j = 0; j < module.platform.length; j++) {
						...
						Object.keys(scope[platform][module.id]).sort().reverse().filter(function (ver) {
							...
							const tmp = Object.assign({}, module, info);
							if (params.sdkVersion && info.manifest && info.manifest.minsdk && version.gt(info.manifest.minsdk, params.sdkVersion)) {
								if (params.logger) {
									params.logger.debug(`Found incompatible Titanium module id=${cyan(tmp.id)} version=${cyan(tmp.version)} platform=${cyan(tmp.platform.join(','))} deploy-type=${cyan(tmp.deployType.join(','))}`);
									params.logger.debug(`Module ${cyan(tmp.id)} requires Titanium SDK ${info.manifest.minsdk} or newer, but the selected SDK is ${params.sdkVersion}`);
								}
							...
							if (platformAPIVersion && modAPIVersion && modAPIVersion !== platformAPIVersion) {
								if (params.logger) {
									params.logger.debug(`Found incompatible Titanium module id=${cyan(tmp.id)} version=${cyan(tmp.version)} platform=${cyan(tmp.platform.join(','))} api-version=${cyan(info.manifest.apiversion)} deploy-type=${cyan(tmp.deployType.join(','))}`);
									params.logger.debug(`Module ${cyan(tmp.id)} has apiversion=${cyan(modAPIVersion)}, but the selected SDK supports module apiversion=${cyan(platformAPIVersion)} on platform=${cyan(platform)}`);
								}
								result.incompatible.push(tmp);
								...
								if (result.found[k].id === tmp.id) {
									...
									if (originalVersion === 'latest') {
										if (version.lt(result.found[k].version, ver)) {
											...
											params.logger && params.logger.info(`Found better matching module id=${cyan(tmp.id)} version=${cyan(originalVersion)} platform=${cyan(tmp.platform.join(','))} deploy-type=${cyan(tmp.deployType.join(','))} path=${cyan(tmp.modulePath)}`);
							...
							if (!alreadyAdded) {
								...
								if (!foundBetter) {
									params.logger?.info(`Found Titanium module id=${cyan(tmp.id)} version=${cyan(tmp.version)} platform=${cyan(tmp.platform.join(','))} deploy-type=${cyan(tmp.deployType.join(','))} path=${cyan(tmp.modulePath)}`);
								}
				...
				if (!found) {
					params.logger && params.logger.warn(`Could not find a valid Titanium module id=${cyan(module.id)} version=${cyan(originalVersion)} platform=${cyan(module.platform.join(','))} deploy-type=${cyan(module.deployType.join(','))}`);
 ...
 * @param {Boolean} [bypassCache=false] - When true, re-scans the specified paths for modules.
 */
export function detect(paramsOrSearchPaths, logger, callback, bypassCache) {
	let params;
...
export async function detectModules(modulesDir, options = {}) {
	...
	logger?.debug(`Detecting modules in ${cyan(modulesDir)}`);
...
async function detectModule(modulePath, ver, ignoreDirs, logger) {
	...
	logger?.debug(`Detected %s module: ${mod.platform[0]} ${cyan(mod.manifest.moduleid)} @ ${mod.modulePath}`);
	return mod;
}
...
async function detectNativeModulesViaNodeModulesDir(nodeModuleDir, logger) {
	logger?.debug(`Detecting modules in ${cyan(nodeModuleDir)}`);
...
async function detectPlatformSpecificNativeModuleInNPMPackage(directory, platformName, platformCount, json, logger) {
	...
	logger?.debug(`Detected ${platform} module: ${cyan(manifest.moduleid)} ${json.version} @ ${modulePath}`);
	return {
