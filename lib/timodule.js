		...
		callback: function (installed) {
			params.modules && params.modules.forEach(function (module) {
				...
				if (!Array.isArray(module.platform)) {
					module.platform = module.platform.split(',');
				}
				...
				if (!Array.isArray(module.deployType)) {
					module.deployType = module.deployType.split(',');
				}
					...
					for (let j = 0; j < module.platform.length; j++) {
						...
						Object.keys(scope[platform][module.id]).sort().reverse().filter(function (ver) {
							...
							if (params.sdkVersion && info.manifest && info.manifest.minsdk && version.gt(info.manifest.minsdk, params.sdkVersion)) {
								if (params.logger) {
									params.logger.debug(`Found incompatible Titanium module id=${tmp.id} version=${tmp.version} platform=${tmp.platform.join(',')} deploy-type=${tmp.deployType.join(',')}`);
									params.logger.debug(`Module ${tmp.id} requires Titanium SDK ${info.manifest.minsdk} or newer, but the selected SDK is ${params.sdkVersion}`);
								}
							...
							if (platformAPIVersion && modAPIVersion && modAPIVersion !== platformAPIVersion) {
								if (params.logger) {
									params.logger.debug(`Found incompatible Titanium module id=${tmp.id} version=${tmp.version} platform=${tmp.platform.join(',')} api-version=${info.manifest.apiversion} deploy-type=${tmp.deployType.join(',')}`);
									params.logger.debug(`Module ${tmp.id} has apiversion=${modAPIVersion}, but the selected SDK supports module apiversion=${platformAPIVersion} on platform=${platform}`);
								}
								result.incompatible.push(tmp);
								...
								if (result.found[k].id === tmp.id) {
									...
									if (originalVersion === 'latest') {
										if (version.lt(result.found[k].version, ver)) {
											...
											params.logger && params.logger.info(`Found better matching module id=${tmp.id} version=${originalVersion} platform=${tmp.platform.join(',')} deploy-type=${tmp.deployType.join(',')} path=${tmp.modulePath}`);
				...
				if (!found) {
					params.logger && params.logger.warn(`Could not find a valid Titanium module id=${module.id} version=${originalVersion} platform=${module.platform.join(',')} deploy-type=${module.deployType.join(',')}`);
