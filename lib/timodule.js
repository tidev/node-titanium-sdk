...
export async function find(modulesOrParams, platforms, deployType, tiManifest, searchPaths, logger, bypassCache) {
	...
	if (typeof options.platforms === 'string') {
		options.platforms = options.platforms.split(',').filter(p => p);
	} else if (Array.isArray(options.platforms)) {
		options.platforms = options.platforms.filter(p => p);
	} else {
		options.platforms = [];
	}
	if (!options.platforms.includes('commonjs')) {
		options.platforms.push('commonjs'); // add commonjs to the list of valid module platforms
	}
	// Align the platform aliases for 'iphone'/'ipad'/'ios' to just be 'ios'
	// and remove duplicates
	options.platforms = Array.from(new Set(options.platforms.map(p => platformAliases[p] || p)));
	...
	if (options.modules) {
		for (const module of options.modules) {
			...
			if (!module.platform) {
				module.platform = options.platforms;
			}
			...
			if (!module.deployType) {
				module.deployType = options.deployType;
			}
			...
			if (!module.deployType.includes(options.deployType)
				|| !module.platform.some(platform => options.platforms.includes(platform))) {
				return;
			}

			// strip all platforms that aren't supported by this build
			for (let i = 0; i < module.platform.length; i++) {
				if (!options.platforms.includes(module.platform[i])) {
			...
			options.logger?.debug(`Looking for Titanium module id=${module.id} version=${originalVersion} platform=${module.platform.join(',')} deploy-type=${module.deployType.join(',')}`);
			...
			for (let i = 0; i < scopes.length; i++) {
				...
				for (let j = 0; j < module.platform.length; j++) {
					...
					for (const ver of sortedVersions) {
						...
						if (options.sdkVersion && info.manifest && info.manifest.minsdk && version.gt(info.manifest.minsdk, options.sdkVersion)) {
							if (options.logger) {
								options.logger.debug(__('Found incompatible Titanium module id=%s version=%s platform=%s deploy-type=%s', tmp.id.cyan, tmp.version.cyan, tmp.platform.join(',').cyan, tmp.deployType.join(',').cyan));
								options.logger.debug(__('Module %s requires Titanium SDK %s or newer, but the selected SDK is %s', tmp.id.cyan, info.manifest.minsdk, options.sdkVersion));
							}
						...
						if (platformAPIVersion && modAPIVersion && modAPIVersion !== platformAPIVersion) {
							if (options.logger) {
								options.logger.debug(__('Found incompatible Titanium module id=%s version=%s platform=%s api-version=%s deploy-type=%s', tmp.id.cyan, tmp.version.cyan, tmp.platform.join(',').cyan, String(info.manifest.apiversion).cyan, tmp.deployType.join(',').cyan));
								options.logger.debug(__('Module %s has apiversion=%s, but the selected SDK supports module apiversion=%s on platform=%s', tmp.id.cyan, String(modAPIVersion).cyan, String(platformAPIVersion).cyan, platform.cyan));
							}
							result.incompatible.push(tmp);
							...
							if (result.found[k].id === tmp.id) {
								...
								if (originalVersion === 'latest') {
									if (version.lt(result.found[k].version, ver)) {
										...
										options.logger?.info(`Found better matching module id=${tmp.id} version=${originalVersion} platform=${tmp.platform.join(',')} deploy-type=${tmp.deployType.join(',')} path=${tmp.modulePath}`);
						...
						if (!alreadyAdded) {
							...
							if (!foundBetter) {
								options.logger?.info(`Found Titanium module id=${tmp.id} version=${tmp.version} platform=${tmp.platform.join(',')} deploy-type=${tmp.deployType.join(',')} path=${tmp.modulePath}`);
							}
			...
			if (!found) {
				options.logger?.warn(`Could not find a valid Titanium module id=${module.id} version=${originalVersion} platform=${module.platform.join(',')} deploy-type=${module.deployType.join(',')}`);
