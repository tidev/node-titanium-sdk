...
async function detectSDK(config, opts) {
				...
				function (cb) {
					findExecutable([ config.get('android.executables.adb'), 'adb' + exe ], function (err, result) {
						if (err) {
							...
							findSDK(expand(result, '..', '..'), config, androidPackageJson, cb);
						}
					});
				}
}
...
export async function detect(config, opts) {
	...
	return Object.assign({}, ...await Promise.all([
		detectJDK(config, opts).then(jdk => ({ jdk }))
	]));
	...
	async.parallel({
		...
		ndk: function (next) {
			...
			queue.push([
				...
				function (cb) {
					findExecutable([ config.get('android.executables.ndkbuild'), 'ndk-build' + cmd ], (err, result) => {
						if (err) {
							cb(err);
						} else {
		...
		linux64bit: function (next) {
			...
			if (process.platform === 'linux' && process.arch === 'x64') {
				...
				async.parallel([
					function (cb) {
						findExecutable([ config.get('linux.dpkg'), 'dpkg' ], (err, dpkg) => {
							if (err || !dpkg) {
								return cb();
							}

							const archs = {};
							run(dpkg, '--print-architecture', (_code, stdout, _stderr) => {
								...
								run(dpkg, '--print-foreign-architectures', (_code, stdout, _stderr) => {
									for (let line of stdout.split('\n')) {
										line = line.trim();
										if (line) {
											archs[line] = 1;
					...
					function (cb) {
						findExecutable([ config.get('linux.dpkgquery'), 'dpkg-query' ], (err, dpkgquery) => {
							if (err || !dpkgquery) {
								return cb();
							}
							...
							async.each(
								...
								function (pkg, next) {
									run(dpkgquery, [ '-l', pkg ], (_code, out, _err) => {
										result[pkg] = false;
										if (!code) {
		...
		const sdkHome = process.env.ANDROID_SDK_HOME && afs.resolvePath(process.env.ANDROID_SDK_HOME);
...
export function findSDK(dir, config, androidPackageJson) {
	if (!dir) {
		return null;
	}
