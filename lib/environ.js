...
export function detectTitaniumSDKs(paths) {
	...
	for (let titaniumPath of sdkPaths) {
		titaniumPath = expand(titaniumPath);
		...
		if (fs.existsSync(titaniumPath)) {
			...
			if (fs.existsSync(mobilesdkPath)) {
				fs.readdirSync(mobilesdkPath).filter((f) => {
					...
					const platforms = sdk.manifest?.platforms || [ 'android', 'ios', 'mobileweb' ];
					for (const p of platforms) {
						...
						if (fs.existsSync(pp)) {
							...
							const dir = path.join(sdkPath, 'iphone');
