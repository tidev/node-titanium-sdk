...
exports.load = function (dir, deviceFamilies, logger, callback) {
	...
	afs.visitFiles(dir, function (file, full) {
		...
		function parseFile(file, name, density) {
			logger.debug(__('Parsing JSS file: %s', file.cyan));
			const rules = parse(fs.readFileSync(file).toString()).stylesheet.rules;
					...
					rule.selectors.forEach(function (selector) {
						...
						rule.declarations.forEach(function (decl) {
							...
							if (/^font-.+/.test(prop)) {
								...
								prop = 'font' + prop.split('-').map(p => p.substring(0, 1).toUpperCase() + p.substring(1)).join('').substring(4);
							}
		...
		if (Object.keys(entries).length) {
			...
			Object.keys(entries).forEach(function (name) {
				...
				Object.keys(entry.density).forEach(d => parseFile(entry.density[d], name, d));
			});
		}
