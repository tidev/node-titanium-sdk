...
or __https://jdk.java.net/archive/__.`
		...
		let { status, stdout, stderr } = spawnSync(jdkInfo.executables.javac, [ '-version', '-d64' ], { encoding: 'utf-8' });
		let arch = '64bit';
		if (status !== 0) {
			// not the 64-bit version, try the 32-bit version
			({ status, stdout, stderr } = spawnSync(jdkInfo.executables.javac, [ '-version' ], { encoding: 'utf-8' }));
			arch = '32bit';
		}
		if (status === 0) {
			const re = /^javac (.+?)(?:_(.+))?$/;
